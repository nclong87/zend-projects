...
define('LIB_DIR', realpath(ROOT_DIR . '/../libs'));
...
function shutDownFunction() {
    ...
    if (isset($error['type']) && $error['type'] == 1) {
        //print_r($error);
        Core\Log::getInstance()->log(array(__FUNCTION__, $error));
    }
}
...
function __autoload($class) {
    //echo $class.PHP_EOL;
    $parts = explode('\\', $class);
    if (in_array($parts[0], array('Core', 'Db', 'Predis', 'Site', 'Tumblr', 'Eher', 'Guzzle', 'Symfony','OAuth'))) {
...
function var_dump_exception($exc,$exit = true) {
    echo '<pre>';
    echo 'Message: <b>'.$exc->getMessage().'</b><br>';
    echo 'Line: <b>'.$exc->getFile().'</b> at line <b>'.$exc->getLine().'</b><br>';
...
function getDB($dbName = 'test2') {
    ...
    if (!isset($storageMaster)) {
        include LIB_DIR . '/Zend/Db.php';
        include LIB_DIR . '/Zend/Loader.php';
...
function is_connected() {
    $connected = @fsockopen("www.haivl.com", 80);
